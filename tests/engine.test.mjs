...
test('rewrite removes unverified time and absolute wrinkle promise',()=>{const r=run('刺激膠原蛋白大量增生，12週徹底消除皺紋');assert.doesNotMatch(r.suggestion.text,/12|徹底|消除|膠原|皺紋/);assert.match(r.suggestion.text,/清爽柔嫩/);assert.equal(r.suggestion.edits.length,2);assert.equal(r.suggestion.edits[0].replacement,null);assert.notEqual(run(r.suggestion.text).risk,'高');});
test('duration paraphrase uses conservative cosmetic wording',()=>{const r=run('使用12週，肌膚看起來更加平滑，皺紋明顯減少');assert.doesNotMatch(r.suggestion.text,/12週|明顯減少|皺紋/);assert.match(r.suggestion.text,/清爽柔嫩/);});
test('unknown text does not become invented efficacy or personal experience',()=>{const r=run('我用了三天，濕疹已根治');assert.doesNotMatch(r.suggestion.text,/我|根治|三天|保濕|撫平/);assert.ok(r.suggestion.edits.every(e=>e.replacement===null));});
test('image reminder stays in rewrite and recheck',()=>{const images=validateImages([{name:'a.png',type:'image/png',size:50}]);const r=run('12週撫平皺紋',{images});assert.match(r.suggestion.note,/圖片尚未分析/);assert.equal(run(r.suggestion.text,{images}).coverage,'text_only_images_pending');});
test('rewrite deduplicates claims and cites the current advertising rule',()=>{const r=run('光滑細緻，肌膚平滑');assert.equal((r.suggestion.text.match(/清爽柔嫩/g)||[]).length,1);assert.deepEqual(r.suggestion.source_ids,['advertising-rules']);});
